...
def cosine_distance(t1, t2):
  return tf.constant(1.0, dtype=tf.float32) - tf.reduce_sum(
    tf.multiply(tf.nn.l2_normalize(t1, axis=1), tf.nn.l2_normalize(t2, axis=1)), axis=1)
...
def hinge_loss(true_ledists, false_ledists, margin):
  return tf.reduce_sum(
    tf.maximum(tf.subtract(tf.constant(margin, dtype=tf.float32), tf.subtract(false_ledists, true_ledists)), 0.0))


class DebbieModel(object):  # setting the adversarial grad scale to -1 turns of the flipping of the gradient
  def __init__(self, embs, mlp_layers, activation=tf.nn.tanh, scope="debbie", reg_factor=0.1, learning_rate=0.0001,
               adversarial=False, adversarial_grad_scale=1.0, batch_size=0):
    ...
    with tf.name_scope(self.scope + "__model"):
      ...
      self.l_e = tf.reduce_sum(
        tf.math.squared_difference(cosine_similarity(self.mapped_target_1, self.mapped_attribute),
